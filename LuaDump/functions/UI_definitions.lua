...
function create_UIBox_character_button(args) 
  ...
  function G.UIDEF.perk()
      G.perk_perkArea = CardArea(
        G.hand.T.x+0,
      ...
      local t = {n=G.UIT.ROOT, config = {align = 'cm', w = 9, h = 4, colour = G.C.CLEAR}, nodes={
                  {n=G.UIT.C, config={align = "cm", w = 9, h = 4, padding = 0.1, emboss = 0.05, r = 0.1, colour = G.C.DYN_UI.BOSS_MAIN}, nodes={
                      {n=G.UIT.R, config={align = "cm", padding = 0.05, minw = 8, minh = 4}, nodes={
                        {n=G.UIT.C, config={align = "cm", padding = 0.2, r=0.2, colour = G.C.L_BLACK, emboss = 0.05, minw = 8, minh = 4}, nodes={
  
                            {n=G.UIT.R, config={align = "cm", maxw = 7}, nodes={
                              {n=G.UIT.T, config={text = localize('k_loottxt'), scale = 1.3, colour = G.C.WHITE, shadow = true}},
                            }},
  
                            {n=G.UIT.R, config={align = "cm", maxw = 7, minh = 4}, nodes={
                              {n=G.UIT.O, config={align = "cm", maxh = 1.05*G.CARD_H, object = G.perk_perkArea}},
                            }},
  
                            {n=G.UIT.R, config={align = "cm", maxw = 1, minh = .5, r=0.15,colour = G.C.FILTER, button = 'skip_perk', hover = true,shadow = true}, nodes = {
                              {n=G.UIT.R, config={align = "cm", padding = 0.07, focus_args = {button = 'x', orientation = 'cr'}, func = 'set_button_pip'}, nodes={
                                {n=G.UIT.R, config={align = "cm"}, nodes={
                                  {n=G.UIT.T, config={text = localize('k_skip'), scale = 0.35, colour = G.C.WHITE, shadow = false}},
                                }},
                              }}
                            }},
                              {n=G.UIT.R, config={align = "cm"}, nodes={
                                  {n=G.UIT.T, config={text = localize('k_skipsub'), scale = 0.3, colour = G.C.WHITE, shadow = false}},
                              }}
...
function create_UIBox_your_collection_jokers()
  ...
  local perk_options = {}
  for i = 1, math.ceil(#G.P_CENTER_POOLS.Perk/(5*#G.your_collection)) do
    table.insert(perk_options, localize('k_page')..' '..tostring(i)..'/'..tostring(math.ceil(#G.P_CENTER_POOLS.Joker/(5*#G.your_collection))))
  end

  for i = 1, 5 do
    for j = 1, #G.your_collection do
      local center = G.P_CENTER_POOLS["Perk"][i+(j-1)*5]
      local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w/2, G.your_collection[j].T.y, G.CARD_W, G.CARD_H, nil, center)
      G.your_collection[j]:emplace(card)
    end
  end
  ...
  local t =  create_UIBox_generic_options({ back_func = 'your_collection', contents = {
        {n=G.UIT.R, config={align = "cm", r = 0.1, colour = G.C.BLACK, emboss = 0.05}, nodes=deck_tables}, 
        {n=G.UIT.R, config={align = "cm"}, nodes={
          create_option_cycle({options = perk_options, w = 4.5, cycle_shoulders = true, opt_callback = 'your_collection_joker_page', current_option = 1, colour = G.C.RED, no_pips = true, focus_args = {snap_to = true, nav = 'wide'}})
        }}
    }})
  return t
end
